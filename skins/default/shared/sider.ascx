...
<div id="Side_Notice" class="sidepanel">
<h4>Notice</h4>
...
<%
    ...
    {
        Writer.Write("<li><a href=\"{0}\" title=\"{1} 于 {2} 发表该日志:{3}\">{4}</a></li>\r\n", Url.BlogUrl(sideblog), sideblog.postuser, sideblog.posttime.ToString("yyyy-MM-dd HH:mm:ss"), sideblog.title, sideblog.title);
    }		    
%>
</ul>
</div>
...
<%	    
    foreach (lb_category category in sideCategoryList) {
        Writer.WriteLine("<li><img src=\"{0}\" border=\"0\" style=\"margin:3px 4px -4px 0px;\" alt=\"{1}\"/><a class=\"CategoryA\" href=\"{2}\" title=\"{1}\">{3}[{6}]</a><a href=\"{4}\" title=\"Rss\" target=\"_blank\"><img src=\"{5}\" border=\"0px\" alt=\"Rss\" /></a></li>\r\n", 
            Url.Content("~/" + skinpath + category.image), 
            category.info, 
            Url.CategoryUrl(category),
            category.name, 
            Url.Action("rss", new { category = category.id.ToString() }), 
            Url.Content("~/" + skinpath + "images/rss.png"), 
            category.num);
    }		    
%>
</ul>
...
<%
    ...
    {
        Writer.Write(string.Format("<dd><a href=\"{0}\" title=\"系统管理\">系统管理</a></dd>\r\n", Url.RouteUrl(new { controller = "admin", action = "login" })));
    }
    if (authority.Post)
    {
        Writer.Write(string.Format("<dd><a href=\"{0}\" title=\"发表新日志\">发表新日志</a></dd>\r\n", Url.RouteUrl(new { controller = "blog", action = "write" })));
        Writer.Write(string.Format("<dd><a href=\"{0}\" title=\"管理草稿箱\">管理草稿箱</a></dd>\r\n", Url.RouteUrl(new { controller = "blog", action = "draft" })));
    }
%>
...
<%
    foreach (lb_friendlink link in links)
    {
        if (link.isimg)
        {
            Writer.Write(string.Format("<dd><a href=\"{0}\" title=\"{1}\" target=\"_blank\"><img src=\"{2}\" alt=\"\" border=\"0px\" /></a></dd>\r\n", link.url, link.intro, link.image));
        }
        else
        {
            Writer.Write(string.Format("<dd><a href=\"{0}\" title=\"{1}\" target=\"_blank\">{2}</a></dd>\r\n", link.url, link.intro, link.name));
        }
    }
%>
